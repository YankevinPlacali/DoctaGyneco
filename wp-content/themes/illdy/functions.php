...
//remove tollbar for all non admin user
...
//Customization function of the default login design
function my_loginInterfaceCss() {
 wp_enqueue_style('custom-login', get_stylesheet_directory_uri() . '/login/login.css');
}
add_action('login_enqueue_scripts', 'my_loginInterfaceCss');
...
function my_customUrlLink(){
 return 'localhost';     //return the home page when clicked of the logo
}
add_filter('login_headerurl', 'my_customUrlLink');

//The title of the logo
function  my_customTitle(){
 return 'Docta Gyneco';
}
add_filter('login_headertitle', 'my_customTitle');
