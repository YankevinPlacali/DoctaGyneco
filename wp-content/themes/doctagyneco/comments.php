<?php if ( post_password_required() ) : ?>
	...
	<div class="enigma_comment_section">	
		<div class="enigma_comment_title">
			...
			<?php echo comments_number(__('No Comments','enigma'), __('1 Commentaire','enigma'), '% Comments').' sur '; ?>&ldquo;<span> <?php the_title(); ?> </span>&rdquo;
		    </h3>
		</div>
...
<div class="enigma_comment_form_section">
	<?php $fields=array(
		'author' => '<div class="enigma_form_group"><label for="exampleInputEmail1">'. __( 'Nom','enigma').'<small>*</small></label><input name="author" id="name" type="text" id="exampleInputEmail1" class="enigma_con_input_control"></div>',
	...
	add_filter('wl_comment_form_default_fields','my_fields');
		...
		'logged_in_as' => '<p class="enigma_logged_in_as">' . __( "Logged in as ",'enigma' ).$user_identity. '.<a href="'. wp_logout_url( get_permalink() ).'" title="Log out of this account">'.__(" Log out?",'enigma').'</a>' . '</p>',
		'title_reply_to' => '<span class="refeman_reply_to">'.__( 'Reply to %s','enigma').'</span>',
		'id_submit' => 'enigma_send_button',
		'label_submit'=>__( 'Poster','enigma'),
		'comment_notes_before'=> '',
		'comment_notes_after'=>'',
		'title_reply'=> '<h2>'.__('Comenter','enigma').'</h2>',		
		'role_form'=> 'form',		
		);		
